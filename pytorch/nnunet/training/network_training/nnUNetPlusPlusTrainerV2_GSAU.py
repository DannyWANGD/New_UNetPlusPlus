...
class nnUNetPlusPlusTrainerV2_GSAU(nnUNetPlusPlusTrainerV2):
    """UNet++ V2 trainer with GSAU skip gating enabled for 2D experiments."""

    def initialize_network(self):
        if self.threeD:
            raise NotImplementedError("nnUNetPlusPlusTrainerV2_GSAU currently supports 2D training only.")
        ...
        self.network = Generic_UNetPlusPlus(self.num_input_channels, self.base_num_features, self.num_classes,
                                            ...
                                            True, True,
                                            use_mlka=False, use_gsau=True,
                                            gsau_kernel_size=7, gsau_init_bias=2.0, gsau_weight_std=1e-3)
